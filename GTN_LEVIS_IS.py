...
def scrap_data(id,pwd,po):
    ...
    try:
        ...
        #Item Scan
        driver.find_element_by_link_text("Item Scan").click()
        driver.find_element_by_name("poNum").send_keys(po)
        driver.find_element_by_xpath("//div[label = 'Ready To Scan']").click()
        time.sleep(10)
        driver.find_element_by_xpath("//*[button = 'OK']").click()
        print("Getting Item Scan Label...")
        time.sleep(2)
...
def gen_excel(barcode,po,so):
    ...
    filepath = "C:\\Users\\"+os.getlogin()+"\\Desktop\\PO_"+str(po)+"_IS.xlsx"
    wb = Workbook() 
    sheet = wb.active
    i = 1
    j = 1
